...
def moving_average(df, n, column_name):

    MA = pd.Series(df[column_name].rolling(n, min_periods=n).mean(), name= column_name + '_MA_' +  str(n))
    df = df.join(MA)
    return df

def exponential_moving_average(df, n, column_name):

    EMA = pd.Series(df[column_name].ewm(span=n, min_periods=n).mean(), name= column_name + '_EMA_' + str(n))
    df = df.join(EMA)
    return df

def momentum(df, n, column_name):

    M = pd.Series(df[column_name].diff(n), name= column_name + '_Momentum_' + str(n))
    df = df.join(M)
    return df

def rate_of_change(df, n, column_name):

    M = df[column_name].diff(n - 1)
    N = df[column_name].shift(n - 1)
    ROC = pd.Series(M / N, name= column_name + 'ROC_' + str(n))
    df = df.join(ROC)
    return df
...
def bollinger_bands(df, n, K, column_name):

    MA = pd.Series(df[column_name].rolling(n, min_periods=n).mean(),name=column_name + '_MA_'+str(n))
    MSD = pd.Series(df[column_name].rolling(n, min_periods=n).std())
    bL = MA - K * MSD
    bL = pd.Series(bL, name=column_name + '_LBB_' + str(n))
    bU = MA + K * MSD
    bU = pd.Series(bU, name= column_name + '_UBB_' + str(n))
    df = df.join(bL)
    df = df.join(MA)
    df = df.join(bU)
    return df

def bollinger_band_perc(df, n, K, column_name):

    MA = pd.Series(df[column_name].rolling(n, min_periods=n).mean(),name=column_name + '_MA_'+str(n))
    MSD = pd.Series(df[column_name].rolling(n, min_periods=n).std())
    bL = MA - K * MSD
    bU = MA + K * MSD
    bP = (df[column_name] - bL)/(bU-bL)
    bP = pd.Series(bP, name= column_name + '_BBPerc_' + str(n))
    df = df.join(bP)
    return df
